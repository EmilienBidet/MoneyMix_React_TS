...
import useMmType from 'auth/hooks/useMmtype';
...
const AccountSettingForm: React.FC<Props> = ({ currentAccount, handleReload, closeSidebar, isFromAccount }) => {
  ...
  const [isManualNoHoldings, setIsManualNoHoldings] = useState<boolean>(false);
  ...
  const { fetchingMmType, mmTypes, mmTypeError } = useMmType();
  const { fetchingMmAssetClass, mmAssetClasses, mmAssetClassError } = useMmAssetClass();
  const { fetchingMmCountry, mmAssetCountries, mmAssetCountryError } = useMmCountry();
  const { fetchingMmRisk, mmAssetRisks, mmAssetRiskError } = useMmRisk();
  ...
  useEffect(() => {
    if (currentAccount) {
      ...
      setIsManualNoHoldings(currentAccount?.isManual && !currentAccount.hasHoldings && currentAccount?.category?.mmCategory !== 'Liabilities')
    }
  }, [currentAccount]);
  ...
  const hasError = error || subTypeError || filterError || mortgageError || loanAccountError || realEstateError || mmTypeError || mmAssetClassError || mmAssetCountryError || mmAssetRiskError;

  const isLoading = fetchingAccountType || fetchingMortgage || fetchingLoanAccount || fetchingRealEstateAccounts || fetchingMmType || fetchingMmAssetClass || fetchingMmCountry || fetchingMmRisk;
    ...
    <Formik
      initialValues={
        ({
          ...
          mmType: currentFormFields?.mmType || '',
          mmAssetClass: currentFormFields?.mmAssetClass || '',
          mmCountry: currentFormFields?.mmCountry || '',
          mmRisk: currentFormFields?.mmRisk || '',
      ...
      onSubmit={async (values, actions) => {
        ...
        if (!isManualNoHoldings) {
          values = omit(values, ['mmType', 'mmAssetClass', 'mmCountry', 'mmRisk'])
        }
    ...
    >
      {(props) => {
        ...
        const setCategory = (cat: string) => {
          if (currentAccount?.isManual && !currentAccount.hasHoldings && (cat === 'Other Assets' || cat === 'Investment Assets')) {
            setIsManualNoHoldings(true);
          } else {
            setIsManualNoHoldings(false);
          }
              ...
              <div className='account-type'>
                <ul className='account-type-list'>
                  ...
                  {isManualNoHoldings &&
                    <>
                      <li>
                        <span className='form-subheading'>Classification Type</span>
                        <SelectInput
                          args={mmTypes}
                          onChange={handleSelectChange}
                          value={values.mmType}
                      ...
                      <li>
                        <span className='form-subheading'>Classification Asset Class</span>
                        <SelectInput
                          args={mmAssetClasses}
                          onChange={handleSelectChange}
                      ...
                      <li>
                        <span className='form-subheading'>Classification Country</span>
                        <SelectInput
                      ...
                      <li>
                        <span className='form-subheading'>Classification Risk</span>
                        <SelectInput
                          args={mmAssetRisks}
                          onChange={handleSelectChange}
                          value={values.mmRisk}
                ...
                <ul className='account-type-list'>
                  ...
                  <li className={hc('maturityDate')}>
                    ...
                    <ReactDatePicker
                      ...
                      onChange={(val: Date) => {
                        setFieldValue('maturityDate', moment(val).toDate());
                      }}
                    />
                  </li>
                ...
                <ul className='account-type-list'>
                  <li className={hc('businessStartDate')}>
                    ...
                    <ReactDatePicker
                      ...
                      onChange={(val: Date) => {
                        setFieldValue('businessStartDate', moment(val).toDate());
                      }}
                    />
                  </li>
