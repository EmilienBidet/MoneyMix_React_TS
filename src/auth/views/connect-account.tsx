...
import { getAccount, getCurrentSubscription, getFastlink, getSubscription, postZaboAccount } from 'api/request.api';
...
export const ConnectAccountMainSection = () => {
  ...
  const handleCryptoExchange = async () => {
    ...
    zaboConfig
      .connect()
      .onConnection(async (account: Record<string, string>) => {
        logger.log('account connected:', account);
        const zaboPayload = {
          id: account.id,
          token: account.token,
        };

        const { error } = await postZaboAccount(zaboPayload);
        setZaboLoading(false);
        if (error) {
          return mmToast('Error on Zabo api call', { type: 'error' });
        }
      ...
      .onEvent((eventName: Record<string, string>, metadata: Record<string, string>) => {
        ...
        setZaboLoading(false);
        logger.log(`[EVENT] ${eventName}`, metadata);
      });
  };
        ...
        <div className='row login-wrapper'>
          ...
          <div className='bg-white credentials-wrapper connect-wrap'>
            <div className='credentials-content connect-account'>
              ...
              <p>
                Money Minx is your place to track everything. Add your accounts, credit cards, investments,
                collectables, real estate and more. Get started by clicking on one of the buttons below.
              </p>
              ...
              <div className='add-account-card'>
                <div className='card-body'>
                  ...
                  <p className='description'>
                    Connect via our aggregation partner to your accounts. Your login credentials are never shared with
                    Money Minx.
                  </p>
              ...
              <div className='add-account-card small crypto'>
                <div className='card-body'>
                  ...
                  <p className='description'>Add your crypto wallets, Bitcoin, Ethereum and other coins. </p>
                  <div className='add-card-bottom'>
                      <button
