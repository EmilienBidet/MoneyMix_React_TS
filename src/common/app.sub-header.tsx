...
const AppSubHeader: React.FC<AppSubHeaderProps> = ({ AccountDetails }) => {
  ...
  useEffect(() => {
    const fetchCurrentAccount = async () => {
      ...
      if (!error) {
        const successAccounts = data.filter(
          (acc: Account) =>
            acc.isManual ||
            acc.mmAccountProvider === 'ZABO' ||
            ...
            (acc.providerAccount?.status === 'SUCCESS' && acc.providerAccount.dataset?.[0]?.nextUpdateScheduled === null)
        );
        const warningAccounts = data.filter(
          (acc: Account) =>
            (!acc.isManual && acc.providerAccount?.status === 'USER_INPUT_REQUIRED') ||
            (!acc.isManual &&
              acc.providerAccount?.status === 'LOGIN_IN_PROGRESS' &&
              acc.providerAccount?.dataset?.[0]?.updateEligibility === 'DISALLOW_UPDATE') ||
            (!acc.isManual &&
              acc.providerAccount?.status === 'SUCCESS' &&
              acc.providerAccount?.dataset?.[0]?.nextUpdateScheduled < moment().toISOString())
        );
        const errorAccounts = data.filter(
          (acc: Account) =>
            !acc.isManual && acc.mmAccountProvider !== 'ZABO' && (acc.providerAccount?.status === 'FAILED' || !acc.providerAccount?.status)
        );
  ...
  return (
    <div className='left-box d-flex align-items-center float-lg-left'>
      <button
        className='mm-btn-primary mm-btn-animate plus-btn'
                        ...
                        <li key={index}>
                          <Link to={`/account-details/${account.id}`} onClick={() => clickElement(dropdownToggle)}>
                            <div className='pr-1'>
                              ...
                              <span>
                                {account.balancesFetchedAt !== null ? getRelativeDate(account.balancesFetchedAt) : ''}
                              </span>
                            </div>
                            <div>
